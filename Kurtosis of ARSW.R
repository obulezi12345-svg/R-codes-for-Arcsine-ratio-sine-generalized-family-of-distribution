...
theta_vals <- seq(0.05, 10, length.out = 50)
lambda_vals <- seq(0.05, 10, length.out = 50)
omega_vals <- seq(0.05, 10, length.out = 50)



# Initialize an empty matrix for Z (kurtosis values)
Z <- matrix(NA, nrow = length(theta_vals), ncol = length(lambda_vals))

# Loop through parameter combinations and compute empirical kurtosis for each combination
for (i in 1:length(theta_vals)) {
  for (j in 1:length(lambda_vals)) {
    ...
    # Compute kurtosis for the simulated data
    Z[i, j] <- e1071::kurtosis(simulated_data)
  }
}
...
# Plotting the 3D surface using persp() for Kurtosis
windows()
par(mfrow=c(1,1))
...
# Plot the 3D surface using persp() for kurtosis
persp(theta_vals, lambda_vals, Z, theta = 30, phi = 30, 
      col = colors, xlab = expression("\u03B8"), ylab = expression("\u03BB"), zlab = "kurtosis",
