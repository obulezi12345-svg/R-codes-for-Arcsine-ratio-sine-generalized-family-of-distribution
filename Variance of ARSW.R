...
generate_data <- function(n, par) {
  ...
  x_vals <- runif(n, min = 0.5, max = 5)  # Generate x values within a range
  data <- sapply(x_vals, function(x) dARSW(par, x))  # Calculate density values
  return(data)
}

# Create grids of parameter values
theta_vals <- seq(0.05, 10.5, length.out = 50)
lambda_vals <- seq(0.05, 10.5, length.out = 50)
omega_vals <- seq(0.05, 10.5, length.out = 50)



# Initialize an empty matrix for variance values
Z_variance <- matrix(NA, nrow = length(theta_vals), ncol = length(lambda_vals))

# Loop through parameter combinations and compute variance for each combination
for (i in 1:length(theta_vals)) {
  for (j in 1:length(lambda_vals)) {
    ...
    # Compute variance for the simulated data
    Z_variance[i, j] <- var(simulated_data)
  }
}
...
persp(theta_vals, lambda_vals, Z_variance, theta = 30, phi =30,
      col = colors, xlab = expression("\u03B8"), ylab =  expression("\u03BB"), zlab = "variance",
      main = "", axes = TRUE, ticktype='detailed')
# Calculate zfacet values
zfacet <- Z_variance[-1, -1]
