...
theta_vals <- seq(0.05, 20, length.out = 50)
lambda_vals <- seq(0.05, 20, length.out = 50)
omega_vals <- seq(0.05, 20, length.out = 50)

# Initialize an empty matrix for Z (skewness values)
Z_skew <- matrix(NA, nrow = length(theta_vals), ncol = length(lambda_vals))

# Loop through parameter combinations and compute skewness for each combination
for (i in 1:length(theta_vals)) {
  for (j in 1:length(lambda_vals)) {
    ...
    # Compute skewness for the simulated data
    Z_skew[i, j] <- skewness_calculation(simulated_data)
  }
}
#alpha = \u03B1
#beta = \u03B2
...
persp(theta_vals, lambda_vals, Z_skew, theta = 30, phi = 30,
      col = colors, xlab = expression("\u03B8"), ylab = expression("\u03BB"), zlab = "Skewness",
